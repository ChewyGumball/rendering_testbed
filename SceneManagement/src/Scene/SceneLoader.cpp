...
std::unordered_map<std::string, std::shared_ptr<TextureBuffer>> loadTextures(rapidjson::Document& json)
{
    ...
    for (auto& texture : json["textures"].GetObject()) {
        ...
        if (texture.value.HasMember("filename")) {
            ...
            textures[name] = std::make_shared<TextureBuffer>(width, height, textureFormats[format], internalTextureFormats[format]);
        }
    }
    return textures;
}
    ...
    for (auto& pass : json["passes"].GetObject()) {
        ...
        if (pass.value.HasMember("bufferOverrides")) {
            ...
            if (overrides.HasMember("colour")) {
                p->setColourBuffer(textures[overrides["colour"].GetString()]);
            }
...
std::unordered_map<std::string, std::shared_ptr<Shader>> loadShaders(rapidjson::Document& json, std::unordered_map<std::string, std::shared_ptr<TextureBuffer>> textures)
{
