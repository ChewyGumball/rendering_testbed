...
namespace Scene {
	class World {
	private:
		std::unordered_map<std::string, std::vector<std::shared_ptr<Renderer::ModelInstance>>> m_modelInstanceGroups;
	...
	public:
		...
		std::vector<std::shared_ptr<Renderer::ModelInstance>> &modelInstances(std::string name);
